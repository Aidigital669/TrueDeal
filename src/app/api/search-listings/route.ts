...
const STOPWORDS = new Set([
  ...
  "listings", "all", "type", "near", "under", "need", "want", "please"
]);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rawTokens = lowerQuery.split(/[^\w\d]+/);
    ...
    const localProducts = await getLocalImportedProducts();
    ...
    try {
      ...
      if (client) {
        ...
        const portfolioConditions = finalTokens.map(token => ({
          $or: [
        ...
        const pDoc = await db.collection("portfolios").findOne({ $or: portfolioConditions });
        if (pDoc) {
          ...
          matchedCompanyProfile = {
            ...
            ownerName: pDoc.ownerName || pDoc.contactPerson || sellerDoc?.ownerName || userDoc?.name || "Abhijit V. (Managing Director)",
        ...
        const orConditions: any[] = finalTokens.map(token => ({
          $or: [
        ...
        if (parsedIntent.location?.city) {
          orConditions.push({ city: { $regex: parsedIntent.location.city, $options: "i" } });
        }
        if (parsedIntent.location?.state) {
          orConditions.push({ state: { $regex: parsedIntent.location.state, $options: "i" } });
        }
        if (parsedIntent.category) {
          orConditions.push({ category: { $regex: parsedIntent.category, $options: "i" } });
        }
    ...
    return NextResponse.json({
      success: true,
      query,
      text: aiOutput.summaryText,
      appliedFilters: aiOutput.appliedFilters,
    ...
    return NextResponse.json({
      success: true,
      query,
      text: "No result found. Please try refining your keywords.",
      appliedFilters: ["⚡ Search Error Recovery"],
