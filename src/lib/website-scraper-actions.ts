...
export async function scrapeAndImportWebsite(targetUrl: string): Promise<ScrapingResult> {
  ...
  logs.push(`[5/5] Ingesting extracted catalog (${products.length} items) directly into MongoDB...`);
  let databaseSaved = 0;
  ...
  try {
    ...
    // 3b. Update Portfolio in MongoDB strictly for this domain's slug
    ...
    // 3c. Insert/Upsert Scraped Products & Categories tagged with sellerId and sellerSlug
    const categoryCache: Record<string, ObjectId> = {};
    ...
    for (const p of products) {
      ...
      if (!categoryCache[catName]) {
        ...
        if (!catDoc) {
          ...
          categoryCache[catName] = catRes.insertedId;
        } else {
          categoryCache[catName] = catDoc._id;
        }
      }
      ...
      const productPayload = {
        ...
        aiSubtext: "AI Ingested & Verified",
        attention: false,
    ...
    logs.push(`[Done] Successfully saved ${databaseSaved} items for seller "${sellerSlug}" into MongoDB database!`);

  } catch (dbErr: any) {
    logs.push(`[MongoDB Notice] ${dbErr.message}. Scraped memory cache synchronized.`);
