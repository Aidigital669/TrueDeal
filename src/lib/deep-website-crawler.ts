...
export async function crawlFullWebsite(targetUrl: string, maxPages: number = 40): Promise<DeepCrawlResult> {
  ...
  // Ingest Genuine Scraped Data into MongoDB Database with strict Seller & Domain Isolation
  logs.push(`[DATABASE SYNC] Ingesting all ${products.length} genuine scraped items into MongoDB collections...`);
  let databaseSaved = 0;

  try {
    const client = await Promise.race([
      clientPromise,
      new Promise<null>((_, reject) => setTimeout(() => reject(new Error("MongoDB connection timeout")), 3000))
    ]);
    ...
    // 3. Upsert Complete Portfolio into MongoDB strictly for this domain's slug
    ...
    logs.push(`[SUCCESS] Fully ingested ${databaseSaved} real properties & projects for seller "${sellerSlug}" into MongoDB!`);
  } catch (dbErr: any) {
